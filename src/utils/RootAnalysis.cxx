...
namespace m2r { extern int main_(int argc, char** argv); }
using namespace std;
...
/// \param [in] runnumbers Pointer to a valid array of desired run numbers to
///        chain together.
/// \param [in] nruns Length of _runnumbers_.
/// \param format String specifying the file name format, uses _printf_ syntax.
///
/// \note Does not return anything, but creates `new` heap-allocated TChains that
///       become part of the present ROOT directory; these must be deleted by the
///       user.
void dragon::MakeChains(const char* prefix, const Int_t* runnumbers, Int_t nruns,
                        const char* format, Bool_t sonik)
{
  if ( !(sonik) ) {
    TChain* chain[] = {new TChain(Form("%s1",  "t"), "Head singles event."),
    ...
    TChain* chain[] = {new TChain(Form("%s0",  "t"), "SONIK event."),
...
////////////////////////////////////////////////////////////////////////////////
/// MakeChains overload
void dragon::MakeChains(const char* prefix, const std::vector<Int_t>& runnumbers,
                        const char* format, Bool_t sonik)
{
...
////////////////////////////////////////////////////////////////////////////////
/// Make chains of dragon event branches from a vector of run numbers
...
void dragon::FriendChain(TChain* chain, const char* friend_name,
                         const char* friend_alias, const char* format,
                         const char* friend_format)
{
  Int_t runnum;
...
dragon::TTreeFilter::TTreeFilter(const char* filename, const char* option,
...
///  <a href=http://root.cern.ch/root/html/TTree.html#TTree:Draw@2> documentation on
...
/// \param parse If true, go ahead and attempt parsing the specified rossum
///        file, if false, a separate call to ParseFile() will be needed.
...
////////////////////////////////////////////////////////////////////////////////
/// Create a ROOT tree of rossum data
...
////////////////////////////////////////////////////////////////////////////////
/// Parse rossum data (`*.rossumdata`) file
...
////////////////////////////////////////////////////////////////////////////////
/// Calculate average current for a given cup reading and cup
...
///     - 3 : FCM
///     - 4 : FCF
/// \param iteration If more than one cup reading was taken, specifices which
///        reading to take the average of (0 == first iteration).
/// \param skipBegin Length of time in seconds to skip at the beginning of the cup
///        readings
/// \param skipEnd Length of time in seconds to skip at the end of the cup readings
/// \returns Average current value for the parameters specified. This is returned
///          as a class that holds both the nominal average value and the 1-sigma
///          error. The 1-sigma error is calculated as the standard deviation of
///          each individual cup reading over the specified duration. Note that to
///          see the mean & error as part of a ROOT session, you can do
/// \code
                                             ...
                                             Double_t skipBegin, Double_t skipEnd)
{
  ...
  const std::vector<Double_t>::iterator::difference_type diffLast =
    std::lower_bound(time.begin(), time.end(), time.back() - skipEnd,
                     std::less_equal<Double_t>()) - time.begin();
...
void dragon::RossumData::ListTrees() const
{
  for (TreeMap_t::const_iterator it = fTrees.begin(); it != fTrees.end(); ++it) {
    std::cout << GetTree_(it)->GetName() << "\t" << GetTree_(it)->GetTitle()
              << ", DATIME: " << GetTime_(it) << "\n";
  }
}
...
void dragon::BeamNorm::CalculateNorm(Int_t run, Int_t chargeState)
{
  ...
  for(Int_t i=0; i< NSB; ++i) {
    if(rundata->sb_counts[i].GetNominal() != 0) {
      ...
      rundata->sbnorm[i] /= (qe * (double)chargeState * (rundata->sb_counts[i] /
                                                         rundata->live_time));
      if(rundata->pressure_full.GetNominal() != 0) {
        rundata->nbeam[i] = (rundata->sb_counts_full[i] / rundata->live_time_tail) *
          rundata->sbnorm[i] / rundata->pressure_full;
      }
    }
  }
}
...
////////////////////////////////////////////////////////////////////////////////
UInt_t dragon::BeamNorm::GetParams(const char* param,
...
/////////////////////// Class dragon::LiveTimeCalculator ///////////////////////
...
Double_t dragon::LiveTimeCalculator::CalculateRuntime(midas::Database* db,
                                                      const char* which,
                                                      Double_t& start,
                                                      Double_t& stop)
{
...
void dragon::LiveTimeCalculator::DoCalculate(Double_t tbegin, Double_t tend)
{
  ...
  for(int i=0; i< 2; ++i) {
    ...
    UInt_t busy_time;
    Double_t trig_time;
    TBranch* trigBranch = get_branch(trees[i], trig_time, "io32.tsc4.trig_time",
                                     "LiveTimeCalculator::DoCalculate");
    TBranch* busyBranch = get_branch(trees[i], busy_time, "io32.busy_time",
                                     "LiveTimeCalculator::DoCalculate");
...
namespace {
  struct AccumulateBusy {
	...
	Output_t operator() (const Output_t& current,
                         const dragon::CoincBusytime::Event& additional);
  };
...
////////////////////////////////////////////////////////////////////////////////
/// Calculate total busy time for coincidence events
///
/// Calcluation of the busy time is performed as follows:
...
/// \param cmd1 "Magnetic constant" for MD1 in (keV/u)*G<sup>2</sup>
/// \param cmd1Err Absolute error on _cmd1_
/// \param temp Ambient temperature in kelvin
dragon::StoppingPowerCalculator::StoppingPowerCalculator(Int_t beamCharge,
                                                         Double_t beamMass,
                                                         Int_t nmol,
                                                         Double_t temp,
                                                         Double_t targetLen,
                                                         Double_t targetLenErr,
                                                         Double_t cmd1,
                                                         Double_t cmd1Err):
  fBeamMass(beamMass),
...
/// \returns Pressure in `dyn/cm<sup>2</sup>`
...
/// \returns Pressure in `dyn/cm<sup>2</sup>` (with uncertainty)
UDouble_t dragon::StoppingPowerCalculator::TorrCgs(UDouble_t torr)
{
  return 10. * torr * 101325. / 760.0;
}
...
/// \returns Density in atoms / cm<sup>2</sup>
...
/// \param nmol Number of atoms per molecule in the gas
/// \param temp Temperature in kelvin
/// \returns Density in atoms / cm<sup>2</sup> (with uncertainty)
...
////////////////////////////////////////////////////////////////////////////////
/// Energy is calculated using equation 2 from
/// <a href="https://www.sciencedirect.com/science/article/pii/S0168900212005773?via%3Dihub">Dave Hutcheon's NIM paper on BGO z-position</a>
/// `E/m = cmd1 * (qB/m)<sup>2</sup> - [1/(2*u*c<sup>2</sup>)] * (E/m)<sup>2</sup>`
///
...
UDouble_t dragon::StoppingPowerCalculator::CalculateEnergy(Double_t md1,
                                                           Double_t md1Err,
                                                           Int_t q, Double_t m,
                                                           Double_t cmd1,
                                                           Double_t cmd1Err)
...
////////////////////////////////////////////////////////////////////////////////
/// \param xaxis Specify the x axis. Valid options are
///        `dragon::StoppingPowerCalculator::kPRESSURE (== 0)` to plot pressure
///        (torr), or `dragon::StoppingPowerCalculator::kDENSITY (== 1)` to
///        plot density (atoms/cm<sup>2</sup>).
/// \param yaxis Specify the y axis. Valid options are
///        `dragon::StoppingPowerCalculator::kMD1 (== 0)` to plot MD1 field
///        (gauss) or `dragon::StoppingPowerCalculator::kENERGY (== 1)` to plot
///        energy (keV/u).
/// \param draw Argument `true` plots the graph with option `"AP"` in addition to
///        returning it.
/// \returns `TGraph` of pressure vs. energy, with asymmetric error bars on both
///          axes. Responsibility is on the user to delete the returned graph.
TGraphAsymmErrors* dragon::StoppingPowerCalculator::PlotMeasurements(XAxisType_t xaxis,
                                                                     YAxisType_t yaxis,
                                                                     Bool_t draw,
                                                                     Bool_t print) const
{
...
UDouble_t dragon::StoppingPowerCalculator::CalculateEbeam(TGraphAsymmErrors** plot)
{
  ...
  for(Int_t i=0; i< GetNmeasurements(); ++i) {
    double md1err = fMd1[i].GetErrLow() < fMd1[i].GetErrHigh() ? fMd1[i].GetErrHigh()
                                          : fMd1[i].GetErrLow();
    pres[i] = fPressures[i];
    energy[i] = CalculateEnergy(fMd1[i].GetNominal(), md1err, fBeamCharge,
                                fBeamMass, fMd1Constant.GetNominal(), 0);
  }
  ...
  if(plot) {
    *plot = (TGraphAsymmErrors*)g->Clone();
    ...
    (*plot)->SetTitle(Form("Beam energy: %.2f^{+ %.3f}_{-%.3f} keV/u; Pressure [Torr]; Energy [keV/u]",
                           out.GetNominal(), out.GetErrHigh(), out.GetErrLow() ) );
...
////////////////////////////////////////////////////////////////////////////////
/// \param [out] plot Address of a pointer to a TGraph that will contain
///        a plot of energy vs. density upon successful return. Passing a
///        non-NULL plot argument also causes the plot to be drawn with option
///        "AP". The default argument (NULL) bypasses any plotting and simply
///        calculates the epsilon parameter.
///
/// \param [out] ebeam Pointer to UDouble_t holding beam energy from offset of
///        linear fit.
///
/// \returns The `epslion` parameter used in calculations of omega-gamma, in
///          units of eV / [atoms / cm<sup>2</sup>]
///
/// \note For treatment of errors, we should only include random errors in the
///       plot to be fit. Systematic errors (i.e target pressure and c_md1) will
///       pull all points in the same direction and should only be added back
///       after the linear fit is done.
                                                            ...
                                                            UDouble_t* ebeam)
{
  ...
  for(Int_t i=0; i< GetNmeasurements(); ++i) {
    ...
    dens[i] = CalculateDensity(fPressures[i], UDouble_t(fTargetLength.GetNominal(), 0),
                               fNmol, fTemp);
    energy[i] = CalculateEnergy(fMd1[i].GetNominal(), md1err, fBeamCharge, fBeamMass,
                                fMd1Constant.GetNominal(), 0);
  }
  // plot and fit to get dE/d[n/A]
...
/// \param epsilon "Stopping power" in e*cm<sup>2</sup>/atom (see dragon::StoppingPowerCalculator)
dragon::ResonanceStrengthCalculator::ResonanceStrengthCalculator(Double_t eres,
                                                                 Double_t mbeam,
                                                                 Double_t mtarget,
                                                                 dragon::BeamNorm* beamNorm,
                                                                 UDouble_t epsilon):
...
UDouble_t dragon::ResonanceStrengthCalculator::CalculateWavelength(UDouble_t eres,
                                                                   Double_t mbeam,
                                                                   Double_t mtarget)
{
...
UDouble_t dragon::ResonanceStrengthCalculator::LambdaSquared(UDouble_t eres,
                                                             Double_t mbeam,
                                                             Double_t mtarget)
{
...
UDouble_t dragon::ResonanceStrengthCalculator::CalculateResonanceStrength(UDouble_t yield,
                                                                          UDouble_t epsilon,
...
void dragon::LabCM::Init(int Zbeam, int Abeam, int Ztarget, int Atarget,
                         double ecm, int qbeam, int qtarget)
{
...
/// \param nmol Number of atoms per molecule of the target (2 for hydrogen, 1
///        for helium).
/// \param temp Target cell temperature in kelvin
/// \param targtLen Effective target length in cm
dragon::CrossSectionCalculator::CrossSectionCalculator(dragon::BeamNorm* beamNorm,
                                                       Int_t nmol,
                                                       Double_t temp,
                                                       Double_t targetLen):
  fBeamNorm(beamNorm),
...
////////////////////////////////////////////////////////////////////////////////
/// \param prefix Desired metrix prefix for the output unit (default = "" is
///        barns, "m" is millibarns, etc.)
/// \param printTotal If true, prints the total cross section to the screen.
///
/// \note This function calculates the run-by-run cross sections for each run
///       that was part of the BatchCalculate() call to the BeamNorm pointer
///       passed to the constructor. The total cross section is the weighted
///       average of these run-by-run measurements.
UDouble_t dragon::CrossSectionCalculator::Calculate(Int_t whichSB, const
                                                    char* prefix, Bool_t printTotal)
{
  if(!fBeamNorm) {
  ...
  while(irun !=  fBeamNorm->GetRuns().end()) {
    ...
    if(rd == 0) break;
    const UDouble_t sigma = CalculateCrossSection(rd->yield[whichSB], rd->pressure,
                                                  fTargetLen, fNmol, fTemp) / prefixval;
    fCrossSections.push_back( sigma );
  }
  fTotalCrossSection = MeasurementWeightedAverage(fCrossSections.begin(),
                                                  fCrossSections.end());
  if(printTotal) {
...
////////////////////////////////////////////////////////////////////////////////
UDouble_t dragon::CrossSectionCalculator::CalculateCrossSection(UDouble_t yield,
                                                                UDouble_t pressure,
                                                                Double_t length,
                                                                Int_t nmol,
                                                                Double_t temp)
{
  UDouble_t rho = dragon::StoppingPowerCalculator::CalculateDensity(pressure, length,
                                                                    nmol, temp);
