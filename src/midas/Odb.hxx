	...
	static bool ReadValue(const char* path, T& value)
		{
			...
			if ( ReadArray(path, &value, sizeof(value)) != 0 )
				...
				return false;
#else
			utils::err::Error("midas::Odb::ReadValue") << "MIDASSYS not defined." << DRAGON_ERR_FILE_LINE;
			return false;
#endif
		}
	...
	static int ReadArray(const char* path, T* array, int length)
		{
			...
			if (status == DB_NO_KEY) {
				utils::err::Error("midas::Odb::ReadArray") <<
			...
			utils::err::Error("midas::Odb::ReadArray") << "Cannot read \"" <<
				path << "\" from odb, status = " << status << DRAGON_ERR_FILE_LINE;
			return 0;
#else
			utils::err::Error("midas::Odb::ReadArray") << "MIDASSYS not defined." << DRAGON_ERR_FILE_LINE;
			return 0;
#endif
		}
...
template<> inline bool Odb::ReadValue<std::string>(const char* path, std::string& value)
{
	...
	status = db_find_key (hdb, hdir, path, &hkey);
	if (status == DB_NO_KEY) {
		utils::err::Error("midas::Odb::ReadValue<std::string>") <<
			"Couldn't get array key for path \"" << path << "\". status = " << status
																					 << DRAGON_ERR_FILE_LINE;
		return 0;
	}
	...
	utils::err::Error("midas::Odb::ReadValue<std::string>") << "Cannot read \"" <<
		path << "\" from odb, status = " << status << DRAGON_ERR_FILE_LINE;
	return false;

#else

	utils::err::Error("midas::Odb::ReadValue<std::string>") << "MIDASSYS not defined." << DRAGON_ERR_FILE_LINE;
	return 0;
...
template<> inline int Odb::ReadArray<std::string>(const char* path, std::string* array, int length)
{
	...
	status = db_find_key (hdb, hdir, path, &hkey);
	if (status == DB_NO_KEY) {
		utils::err::Error("midas::Odb::ReadArray<std::string>") <<
			"Couldn't get array key for path \"" << path << "\". status = " << status
																					 << DRAGON_ERR_FILE_LINE;
		return 0;
	}
	...
	return false;
#else
	utils::err::Error("midas::Odb::ReadArray<std::string>") << "MIDASSYS not defined." << DRAGON_ERR_FILE_LINE;
	return 0;
#endif
}
