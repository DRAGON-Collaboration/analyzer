	...
	static bool ReadValue(const char* path, T& value)
		{
			...
			if ( ReadArray(path, &value, sizeof(value)) != 0 )
				...
				return false;
#else
			dragon::err::Error("midas::Odb::ReadValue") << "MIDASSYS not defined." << DRAGON_ERR_FILE_LINE;
			return false;
#endif
		}
	...
	static int ReadArray(const char* path, T* array, int length)
		{
			...
			return 0;
#else
			dragon::err::Error("midas::Odb::ReadArray") << "MIDASSYS not defined." << DRAGON_ERR_FILE_LINE;
			return 0;
#endif
		}
...
template<> inline bool Odb::ReadValue<std::string>(const char* path, std::string& value)
{
	...
	dragon::err::Error("midas::Odb::ReadValue<std::string>") << "MIDASSYS not defined." << DRAGON_ERR_FILE_LINE;
	return 0;
...
template<> inline int Odb::ReadArray<std::string>(const char* path, std::string* array, int length)
{
	...
	return false;
#else
	dragon::err::Error("midas::Odb::ReadArray<std::string>") << "MIDASSYS not defined." << DRAGON_ERR_FILE_LINE;
	return 0;
#endif
}
