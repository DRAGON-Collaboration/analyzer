...
MAKE_DRAGON_DICT += rootcling -v -f $@ -s $(SHLIBFILE) -rml $(SHLIBFILE) -rmf $(ROOTMAPFILE) -c $(CXXFLAGS) -I[$(INCLUDES)] -p $(HEADERS) TTree.h $(CINT)/Linkdef.h
...
clean:
	rm -rf $(DRLIB)/*.so $(DRLIB)/*.pcm $(DRLIB)/*.rootmap $(OBJECTS) $(RB_DRAGON_OBJECTS)  $(RB_SONIK_OBJECTS) obj/rootana/*.o obj/*/*.o $(CINT)/*.cxx $(CINT)/rootana/* bin/*
