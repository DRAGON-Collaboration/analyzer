...
pydragon: $(SRC)/pydragon.cxx $(SRC)/pydragon.hxx $(SHLIBFILE)
	$(CXX) -O2 -Wall -shared -std=c++11 -fPIC -static-libstdc++ $(shell python3 -m pybind11 --includes) \
${PWD}/src/pydragon.cxx -o pydragon$(shell python3-config --extension-suffix) -I${PWD}/src -L${PWD}/lib -lDragon
...
.PHONY: clean
clean: $(CLEAN_ALL)
	rm -f $(DRA_DICT) $(SHLIBFILE) $(ROOTMAPFILE) $(OBJECTS) $(RB_DRAGON_OBJECTS) $(RB_SONIK_OBJECTS) $(DRLIB)/*.so $(DRLIB)/*.pcm $(DRLIB)/*.h $(PWD)/bin/mid2root pydradon*.so
