...
ifeq ($(USE_ROOT),YES)
ifdef ROOTSYS
ROOTLIBS= -L$(ROOTSYS)/lib $(shell $(ROOTSYS)/bin/root-config --cflags --libs --glibs) -I$(ROOTSYS)/include
else
...
ROOTANA_OBJS=$(ROOTANA)/libNetDirectory/netDirectoryServer.o
ROOTANA_FLAGS=-ansi -Df2cFortran -DHAVE_LIBNETDIRECTORY -I$(ROOTANA) $(ROOTANA)/libNetDirectory/netDirectoryServer.o

$(OBJ)/rootana/%.o: $(SRC)/rootana/%.cxx $(SRC)/rootana/*.hxx
	$(CXX) -c \
-o $@ -p $< \

anaDragon: $(SRC)/rootana/anaDragon.cxx $(DRLIB)/libDragon.so $(ROOTANA_OBJS)
	$(LINK) $(RBINC) $(ROOTANA_FLAGS) \
\
-o $@ -p $< \
\
$(ROOTANA_OBJS) $(MIDASLIBS) -lDragon -L$(DRLIB)


rootanaTimestamp: $(OBJ)/rootana/%.o
...
clean:
	rm -f $(DRLIB)/*.so $(CINT)/DragonDictionary.* $(OBJECTS) $(RB_OBJECTS) anaDragon
